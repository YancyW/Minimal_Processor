...
void plot_2D_vec(std::vector<std::string> input_name,std::string title_name,std::string title_name1,std::string axis_name,std::string axis_name1,std::vector<float> region,std::vector<string> axistype) {
	...
	for(int i=0;i<filename.size();i++){
		...
		for(int event=0;event<nevent;event++){
			...
			for(unsigned int vsize=0;vsize<min(variable1->size(),variable->size());vsize++){
				fvariable[i]->Fill(variable->at(vsize),variable1->at(vsize));
			}
		}
